...
- 在豆包输入法中设置语音输入快捷键。
- 打开应用授权辅助功能和输入监控。
- 重启应用，在应用内按需把“长按模式”“免按模式”或“单击模式”的快捷键设置为与豆包语音输入快捷键一致。
- 开始使用：
  ...
  - 单击模式用于“单击开始，再次单击结束”。第一次单击快捷键后，应用会临时切换到豆包输入法，并代为保持豆包语音快捷键按下；再次单击同一快捷键后，应用会释放该快捷键并恢复原输入法。该模式默认未设置，需要在应用内主动选择快捷键后启用。
