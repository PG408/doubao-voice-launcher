...
- 支持长按模式：按住指定修饰键超过约 `0.10s` 后切换到豆包输入法，并向豆包转发一次快捷键按下；松开后转发快捷键抬起，并恢复原输入法。
- 支持免按模式：在约 `0.45s` 内双击指定修饰键后切换到豆包输入法，并在约 `0.10s` 后向豆包转发一次快捷键按下；再次按下该快捷键时转发快捷键抬起，并恢复原输入法。
- 支持在应用内选择快捷键。
...
- 在豆包输入法中设置语音输入快捷键。
- 打开应用授权辅助功能和输入监控。
- 重启应用，在应用内把“长按模式”和“免按模式”的快捷键设置为与豆包语音输入快捷键一致。
- 开始使用：
  - 长按模式用于“按住说话，松手结束”。应用会在按住期间临时切换到豆包输入法，并在松开后恢复原输入法。
  - 免按模式用于“免持续按住说话”。双击快捷键后，应用会临时切换到豆包输入法，并代为保持豆包语音快捷键按下；再次按下快捷键后，应用会释放该快捷键并恢复原输入法。
